...
for train_idx, test_idx in tqdm(logo.split(X, y, groups)):
    ...
    y_train_standard = (y_train - y_train.min()) / (y_train.max() - y_train.min()) # changed in the nominator the mean to min
    y_test_standard = (y_test - y_train.min()) / (y_train.max() - y_train.min()) # changed in the nominator the mean to min
...
results_df.to_csv('XGB_random_grouping_constrained_similar_size_all_metrics.csv', index=False)