...
# Randomly select 10 unique site IDs (without replacement)
random_10_sites = np.random.choice(all_sites, size=10, replace=False)
print(random_10_sites)

# Filter the DataFrame for these random site IDs
subset_df = df[df['site_id'].isin(random_10_sites)]
...
def run_fold(fold_index):
    ...
    os.makedirs("results_independent_scaling_random_10_sites", exist_ok=True)
