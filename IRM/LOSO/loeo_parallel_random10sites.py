...
# Take same random sites as in /cluster/project/math/akmete/MSc/IRM/LOSO/loeo_parallel_independent_scaling_random10sites.py
# to be able to compare independent scaling with non-independent scaling with respect to training environments
# preferably (to enforce full domain generalization) independent scaling
sites = ['DE-Lnf','US-Wi8','CA-Qfo','US-ARM','IT-MBo','GF-Guy','US-NC4','FR-Hes','AU-GWW','DE-SfN']

# Filter the DataFrame for these random site IDs
subset_df = df[df['site_id'].isin(sites)]
...
# Prepare the environments list (each environment (X,y) corresponds to a site)
environments = []
...
##########################################
### DEFINE GLOBAL SCALING FUNCTIONS  #####
##########################################

def scale_environments_global(train_environments, test_environments):
    """
    Pools all training environments together to compute global min and max,
    and then scales both training and test environments using these parameters.
    """
    ...
    # Using a small epsilon for the target scaling denominator
    epsilon = 1e-8
    ...
    # Scale test environments using the same training parameters
    scaled_test_envs = []
...
class IRM:
    def __init__(self, environments, args):
        best_reg = 0
        best_err = 1e6
        # Use the last environment for validation
...
# Use the random sites as site_ids for folds (they are the unique values in df)
...
def run_fold(fold_index):
    ...
    # Scale the data globally (pooling training environments together)
    scaled_train_envs, scaled_test_envs = scale_environments_global(train_environments, test_environment)
    
    # Train the model on the scaled training environments
    irm_model = IRM(scaled_train_envs, args)
    
    # Evaluate on the left-out (scaled) test environment
    ...
    os.makedirs("results_global_scaling_random_10_sites", exist_ok=True)
    output_filename = os.path.join("results_global_scaling_random_10_sites", f"fold_{fold_index}_results.csv")
