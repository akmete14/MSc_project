...
# Define parameters and initialize for LOGO
args = {
...
for i in range(len(environments)):
    ...
    # Sitename which is left out in this loop and being tested on
    site_id = site_ids[i]
    ...
    # Evaluate on left-out environment
    x_test, y_test = scaled_test_envs[0]
    y_pred_scaled = x_test @ irm_model.solution()

    # Get metrics
    test_mse = ((y_pred_scaled - y_test) ** 2).mean().item()
    print(f"Test MSE (Environment {i} left out): {test_mse:.5f}")
    test_rmse = numpy.sqrt(test_mse)
    print(f"Test RMSE (Environment {i} left out): {test_rmse:.5f}")

    metrics.append(site_id, test_mse, test_rmse)

results = pd.DataFrame(metrics)
results.to_csv('Leave-One-Environment-Out_100sites.csv', index=False)