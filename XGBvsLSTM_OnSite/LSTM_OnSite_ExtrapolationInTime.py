...
output_file = 'extrapolation_in_time_single_site.csv'
...
for site in tqdm(sites):
    ...
    # 1) Get train and test of feature and target
    X_train_time_raw = df_train.drop(columns=['GPP'])
    y_train_time_raw = df_train['GPP']
    X_test_time_raw  = df_test.drop(columns=['GPP'])
    y_test_time_raw  = df_test['GPP']
    ...
    # shapes
    print(X_train_time_seq.shape)  # (N_train_samples, seq_len, num_features)
    print(y_train_time_seq.shape)  # (N_train_samples,)
    print(X_test_time_seq.shape)   # (N_test_samples, seq_len, num_features)
    print(y_test_time_seq.shape)   # (N_test_samples,)
    ...
    # Train on chronological split data
    history = model.fit(
    ...
    # 1) MSE (returned by model.evaluate is the loss function -> MSE in scaled domain)
    ...
    print(f"Metrics for site {site}: Loss={test_loss}, MSE={mse}, R2={r2}, Relative Error={relative_error}, MAE={mae}, RMSE={rmse}")
