...
# Given screened features, generate all possible subset of features
all_subsets = []
...
# Define functioin that computes the pred and stab scores
...
# For each subset, calculate pred and stab score
pred_scores_all = []
...
# First we filter the subsets satisfying stability threshold
alpha_pred = 0.05
quantile_level = 0.1
...
# From the stable subsets, we keep those subsets that satisfy additionally a prediction threshold
...
# Given O_hat, we train the models
trained_models = {}
for subset in O_hat:
...
# Given trained models, each gets the same weight in the ensemble model
weight = 1.0 / len(O_hat) if len(O_hat) > 0 else 0

# Evaluate (according to Equation (1.1)) the ensemble predictor
...
# For comparison, evaluate the full model on the screened features
...
# Evaluate both models on the test site
...
# If needed, save plots of histo of pred scores and stab scores
plt.figure()
