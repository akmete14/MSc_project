...
df_xgb_nostab = pd.read_csv('/cluster/project/math/akmete/MSc/Stabilized_Regression/LOSO/NoStability/XGBoost/plots_and_results_50balanced/results_50sites.csv')
df_xgb_withstab = pd.read_csv('/cluster/project/math/akmete/MSc/Stabilized_Regression/LOSO/WithStability/XGBoost/plots_and_results_50balanced/results_50sites.csv')

# Column names of both dataframes
# no stability: test_site,ensemble_mse_scaled,full_mse_scaled,ensemble_rmse_scaled,full_rmse_scaled,ensemble_r2,full_r2,ensemble_relative_error,full_relative_error,ensemble_mae,full_mae,O_hat_count
# with stability: test_site,ensemble_mse_scaled,full_mse_scaled,ensemble_rmse_scaled,full_rmse_scaled,ensemble_r2,full_r2,ensemble_relative_error,full_relative_error,ensemble_mae,full_mae,G_hat_count,O_hat_count,O_hat
...
df_xgb_nostab = df_xgb_nostab[['test_site', 'ensemble_rmse_scaled', 'full_rmse_scaled', 'Stability']]
df_xgb_withstab = df_xgb_withstab[['test_site', 'ensemble_rmse_scaled', 'full_rmse_scaled', 'Stability']]

# Create a third dataset for the full model RMSE (values are the same in both datasets)
df_full_rmse = df_xgb_nostab[['test_site', 'full_rmse_scaled']].copy()
...
# Show the plot
plt.savefig('boxplot_xgb.png', dpi=300)
