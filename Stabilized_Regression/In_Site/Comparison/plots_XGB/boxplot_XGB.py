...
df_xgb_nostab = pd.read_csv('/cluster/project/math/akmete/MSc/Stabilized_Regression/In_Site/NoStability/XGBoost/results_screened_O_hat_modified_top7.csv')
df_xgb_withstab = pd.read_csv('/cluster/project/math/akmete/MSc/Stabilized_Regression/In_Site/WithStability/XGBoost/results_screened_all_metrics_modified.csv')
...
df_xgb_nostab = df_xgb_nostab[['site', 'ensemble_rmse_scaled', 'full_rmse_scaled', 'Stability']]
df_xgb_withstab = df_xgb_withstab[['site', 'ensemble_rmse_scaled', 'full_rmse_scaled', 'Stability']]

# Create a third dataset for the full model RMSE (values are the same in both datasets)
df_full_rmse = df_xgb_nostab[['site', 'full_rmse_scaled']].copy()
...
# Show the plot
plt.savefig('boxplot_XGB_modified.png')
