...
parser = argparse.ArgumentParser(description="Insite Stabilized Regression with Linear Regression")
...
def compute_scores_insite(df_site, feature_subset, target_column):
    ...
    # Use LinearRegression instead of XGBoost
...
quantile_level = 0.1  # consider subsets with stability above the 90th percentile
alpha_stab = np.quantile(stab_scores_all, quantile_level)
print("Stability threshold (alpha_stab):", alpha_stab)

# Select subsets based on thresholds:
...
print("O_hat subsets for site", site, ":", O_hat)  # Print the selected feature subsets
...
# Train a full model using all features on the training portion for comparison
...
# Compute metrics of full model (i.e all features used)
full_mse_scaled = mean_squared_error(y_test_scaled_full, full_preds_scaled)
