...
df_lr_nostab = pd.read_csv('/cluster/project/math/akmete/MSc/Stabilized_Regression/LOGO/NoStability/LR/results_lasso_withOhat.csv')
df_lr_withstab = pd.read_csv('/cluster/project/math/akmete/MSc/Stabilized_Regression/LOGO/WithStability/LR/results_lasso.csv')
...
df_lr_nostab = df_lr_nostab[['test_cluster', 'ensemble_rmse_scaled', 'full_rmse_scaled', 'Stability']]
df_lr_withstab = df_lr_withstab[['test_cluster', 'ensemble_rmse_scaled', 'full_rmse_scaled', 'Stability']]

# Create a third dataset for the full model RMSE (values are the same in both datasets)
df_full_rmse = df_lr_nostab[['test_cluster', 'full_rmse_scaled']].copy()
...
# Show the plot
plt.savefig('boxplot_logo_LR.png')
