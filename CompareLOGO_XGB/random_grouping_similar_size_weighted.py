...
df = pd.read_csv('/cluster/project/math/akmete/MSc/LOGO/LOGOCV_random_grouping_constrained_similar_size_WeightedTraining_results.csv')

# 1) Ensure the "plots" folder exists
if not os.path.exists("plots_random_grouping_similar_size_weighted"):
    os.makedirs("plots_random_grouping_similar_size_weighted")
...
for metric in metrics:
    ...
    plt.tight_layout()
    plt.savefig(f"plots_random_grouping_similar_size_weighted/{metric}_barplot.png")
...
plt.ylabel("RMSE")
plt.tight_layout()
plt.savefig("plots_random_grouping_similar_size_weighted/RMSE_boxplot.png")
plt.close()