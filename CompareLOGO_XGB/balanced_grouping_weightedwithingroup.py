...
df = pd.read_csv('/cluster/project/math/akmete/MSc/LOGO_test/LOGOCV_WeightedTraining_weighted_within_group_results.csv')

# 1) Ensure the "plots" folder exists
if not os.path.exists("plots_balanced_grouping_weightedwithingroup"):
    os.makedirs("plots_balanced_grouping_weightedwithingroup")
...
for metric in metrics:
    ...
    plt.tight_layout()
    plt.savefig(f"plots_balanced_grouping_weightedwithingroup/{metric}_barplot.png")
...
plt.ylabel("RMSE")
plt.tight_layout()
plt.savefig("plots_balanced_grouping_weightedwithingroup/RMSE_boxplot.png")
plt.close()