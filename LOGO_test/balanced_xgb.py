# Running same as /cluster/project/math/akmete/MSc/LOGO/XGB_weighted.py but now also weighted onsite,
# so that each site within a group contributes the same to the group weight. Like that, we want to see
# whether we still have such a small RMSE for group 7 & 4 in /cluster/project/math/akmete/MSc/CompareLOGO_XGB/plots_balanced_grouping/RMSE_barplot.png
...
print(results_df)
results_df.to_csv("LOGOCV_WeightedTraining_weighted_within_group_results.csv", index=False)