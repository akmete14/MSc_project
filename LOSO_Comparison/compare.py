...
# Read CSV files
df_lr = pd.read_csv('/cluster/project/math/akmete/MSc/LR/LOSO/results.csv')
df_xgb = pd.read_csv('/cluster/project/math/akmete/MSc/XGBoost/LOSO/results.csv') 
# df_lstm = pd.read_csv('/cluster/project/math/akmete/MSc/LSTM/LOSO/results_LOSO.csv')
...
rmse_data = pd.DataFrame({
    'Model': (['Linear Regression'] * len(df_lr) +
              ...
              #['SR (No Stability)'] * len(df_sr_nostab_filtered) +
              #['SR (With Stability)'] * len(df_sr_withstab_filtered)
            ),
    'RMSE': pd.concat([
              df_lr['rmse'],
              df_xgb['rmse'],
              ...
              #df_sr_nostab_filtered['ensemble_rmse'],
              #df_sr_withstab_filtered['ensemble_rmse']
          ], ignore_index=True)
})
...
plt.ylabel('RMSE')
plt.tight_layout()
plt.savefig('boxplot_loso_filtered.png')


# df_lr columns are: site_left_out,mse,rmse,r2
# df_xgb columns are: site,mse,rmse,r2_score,relative_error,mae
