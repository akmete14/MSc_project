...
def process_group(group, sites, files, coords, path):
    ...
    if isinstance(pipeline['regressor'], LinearRegression):
        feature_importance = pipeline['regressor'].coef_
        feature_importance_df = pd.DataFrame({
            'Feature': X_train_all.columns,
            'Importance': feature_importance
        }).sort_values(by='Importance', ascending=False)
        print(f"Feature importance for Linear Regression (Group {group}):")
...
if __name__ == "__main__":
    ...
    results_df.to_csv('LOGOCVLRwithoutlag_RandomGroupswithfeatimportance.csv', index=False)
